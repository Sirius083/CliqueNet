...
# Note:(Sirius)
# 1. add loss and accuracy summary
# 2. train from saved ckpt file
...
###     add summary(sirius)       
        loss_summary = tf.summary.scalar('loss',loss_cross_entropy) 
        accuracy_summary = tf.summary.scalar('accuracy',accuracy)   
        merged = tf.summary.merge_all()
        # summary_dir = args.result_dir


###     optimizer
        optimizer = tf.train.MomentumOptimizer(learning_rate, 0.9, use_nesterov=True)
        train_op = optimizer.minimize(loss_cross_entropy + l2_loss*weight_decay, global_step=tf.train.create_global_step())
        # add global step(sirius)
...
###     begin training     ###    
    ...
    with tf.Session(config=config, graph=graph) as sess:
        ...
        # restore if ckpt file exists
        if tf.train.latest_checkpoint(result_dir):
           saver.restore(sess, tf.train.latest_checkpoint(result_dir))

        train_writer = tf.summary.FileWriter(os.path.join(result_dir, 'train'), graph)
        test_writer = tf.summary.FileWriter(os.path.join(result_dir, 'test'))
        ...
        for epoch in range(1, total_epoches+1):
            ...
            for batch_id in range(batch_count):
                ...
                result_per_bat = sess.run([train_op, loss_cross_entropy, accuracy, merged, tf.train.get_global_step()],
                ...
                # sirius
                summary = result_per_bat[3]
                step = result_per_bat[4]
                
                # save train summary every step
            ...
            # add test statistics to graph
            loss_test_epoch = np.mean(loss_per_bat)
            accuracy_test_epoch = np.mean(acc_per_bat)

            # add calculated statistics
            summary = tf.Summary()
            summary.value.add(tag = 'loss test', simple_value = loss_test_epoch)
            summary.value.add(tag = 'accuracy test', simple_value = accuracy_test_epoch)
