...
def unpickle(file):
    # import cPickle python
    import _pickle as cPickle
    fo = open(file, 'rb')
    dict = cPickle.load(fo,encoding='latin1') # python3 cPickle change encoding
    fo.close()
    if 'data' in dict:
...
def load_data10(phase, data_path):
    ...
    labels = np.array([ [ float(i == label) for i in range(10) ] for label in labels ])   ## cifar-10
    return data, labels
...
def svhn_preprocess(train_path, extra_path, test_path):
    ...
    train_labels_0=np.array([[float(i==label) for i in range(10)] for label in train_labels_0])
    ##
    ...
    extra_labels=np.array([[float(i==label) for i in range(10)] for label in extra_labels])
    ##
    ...
    test_labels=np.array([[float(i==label) for i in range(10)] for label in test_labels])    
